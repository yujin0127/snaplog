# ---------------------------------------------
# Snaplog Full Backend
# Flask + CosmosDB for NoSQL + Auth + Entries + Auto Diary (OpenAI)
# ---------------------------------------------
...
# ---------------------------------------------
# CosmosDB 연결 (for NoSQL)
# ---------------------------------------------
COSMOS_URI = os.environ.get("COSMOS_URI", "YOUR_COSMOS_URI")
COSMOS_KEY = os.environ.get("COSMOS_KEY", "YOUR_COSMOS_KEY")
DB_NAME = "snaplog-db"

client_cosmos = CosmosClient(COSMOS_URI, credential=COSMOS_KEY)
db = client_cosmos.create_database_if_not_exists(id=DB_NAME)

# Users container
users_container = db.create_container_if_not_exists(
    id="users",
    partition_key=PartitionKey(path="/userNum"),
)

# Entries container
entries_container = db.create_container_if_not_exists(
    id="entries",
    partition_key=PartitionKey(path="/userNum"),
)
...
ALT_TEXT_MODEL = os.getenv("OPENAI_ALT_TEXT_MODEL", "gpt-4o")  # 초안 2차
...
# 원본 저장 디렉터리 (멀티파트 업로드용, 필요 없으면 폴더만 만들어두면 됨)
...
def new_user_num() -> int:
    # 전체 유저 수 + 1 을 userNum으로 사용
...
def signup():
    ...
    if not email or not password:
        return jsonify({"ok": False, "error": "이메일/비밀번호 필요"}), 400
    ...
    if exists:
        return jsonify({"ok": False, "error": "이미 존재하는 이메일"}), 400
    ...
    users_container.create_item(doc)
    session["user_id"] = user_id

    return jsonify({"ok": True, "user": {"email": email, "userNum": user_num}})
...
def login():
    ...
    if not items:
        return jsonify({"ok": False, "error": "존재하지 않는 계정"}), 400

    user = items[0]
    if not check_password_hash(user.get("passwordHash", ""), password):
        return jsonify({"ok": False, "error": "비밀번호 오류"}), 400
...
# ---------------------------------------------
# Entries: 일기 조회 / 저장 / 삭제 (Cosmos)
# ---------------------------------------------
@app.get("/api/entries")
@login_required
def get_entries(current_user):
    user_num = current_user["userNum"]
    query = f"SELECT * FROM c WHERE c.userNum={user_num}"
    docs = list(
        entries_container.query_items(
            query=query, enable_cross_partition_query=True
        )
    )
    # 최신 저장 순 (tn 기준 내림차순)
    docs.sort(key=lambda x: x.get("tn", 0), reverse=True)
    return jsonify({"ok": True, "entries": docs})


@app.post("/api/entries")
@login_required
def save_entry(current_user):
    ...
    doc = {
        ...
        "photoItems": payload.get("photoItems", []),
        "repIndex": payload.get("repIndex", 0),
        "date": payload.get("date", ""),
        "ts": payload.get("ts", 0),
        "tn": now_ms(),
    }

    entries_container.upsert_item(doc)
    return jsonify({"ok": True, "entry": doc})
...
def delete_entry(current_user, entry_id):
    user_num = current_user["userNum"]
    try:
        entries_container.delete_item(item=entry_id, partition_key=user_num)
        return jsonify({"ok": True})
    except Exception as e:
        print("delete error:", e)
        return jsonify({"ok": False, "error": "삭제 실패"}), 400


# ========================================================
# 이하: 기존 server.py의 자동 일기 생성 로직 통째로 통합
# ========================================================
    ...
    while total_wait <= MAX_WAIT_SECONDS:
        ...
        with _throttle_lock:
            try:
                ...
                last_error = e
                msg = str(e) or ""
                retry_ms_match = re.search(
                    r"try again in\s+(\d+)\s*ms", msg, re.I
                )
                if retry_ms_match:
                    retry_secs = max(
                        retry_secs,
                        float(retry_ms_match.group(1)) / 1000.0,
                    )
                else:
...
# ---------------- 금지/정리 유틸 ----------------
FILE_RE = re.compile(
    r"\b[\w\-]+\.(jpg|jpeg|png|webp|heic)\b", re.I
)
DATE_RE = re.compile(
    r"\b20\d{2}\s*[-.]?\s*\d{1,2}\s*[-.]?\s*\d{1,2}\b|\b20\d{2}\s*년\s*\d{1,2}\s*월\s*\d{1,2}\s*일\b"
)
...
def replace_proper_nouns_if_no_visible_text(
    analysis: dict, draft: str
) -> str:
    if not analysis or not draft:
        return draft
    frames = analysis.get("frames") or []
    any_visible_text = any(
        f.get("visible_text", "").strip() for f in frames
    )
    if any_visible_text:
        return draft
...
# ---------------- 카테고리 ----------------
FOOD_RE = re.compile(
    r"(음식|식당|카페|요리|coffee|cafe|cake|bread|meal|lunch|dinner|brunch|dessert|커피|빵|케이크|디저트)",
    re.I,
)
...
def compose_from_frames(analysis: dict) -> str:
    ...
    breaks = {
        pos: diff
        for (pos, diff) in _day_break_positions(
            (analysis or {}).get("date_sequence") or []
        )
    }
    pieces = []
    for idx, f in enumerate(frames, start=1):
        ...
        io_flag = f.get("indoor_outdoor") or ""
...
_TIME_SHIFT_PAT = re.compile(
    r"(다음\s*날|이틀\s*뒤|사흘\s*뒤|\d+\s*일\s*뒤|며칠\s*후)", re.I
)
_TAG_RE = re.compile(r"</?f(\d+)>", re.I)


def _reorder_by_tags(
    text: str, n_frames: int, date_sequence: list[str]
) -> str | None:
    ...
    breaks = {
        pos: diff
        for (pos, diff) in _day_break_positions(date_sequence or [])
    }
    out_parts = []
    for i in range(1, n_frames + 1):
        if i in breaks:
...
def analyze_images(
    ...
    photos_summary: list[dict] | None = None,
) -> dict | None:
    if not images:
        return None
    ...
    for idx, img in enumerate(images[:MAX_IMAGES]):
        ...
        if isinstance(img, dict):
            ...
            client_ts = next(
                (img.get(k) for k in cand_img_keys if img.get(k) is not None),
                None,
            )
        ...
        if dt is None and photos_summary and idx < len(photos_summary):
            ...
            ps_time = next(
                (ps.get(k) for k in cand_ps_keys if ps.get(k)),
                None,
            )
            if ps_time:
        ...
        if (
            ...
            and img_data.startswith("data:image")
        ):
            try:
                image_data = (
                    img_data.split(",")[1]
        ...
        print(
            f"[analyze_images] idx={idx}, source={src}, dt={dt.isoformat() if dt else 'None'}"
        )

    images_with_time.sort(
        key=lambda x: (
            x["datetime"] is None,
    ...
    prompt = (
        ...
        "- 각 사진에 대해: 핵심 한줄(summary), 보이는 요소(elements), 실내/실외(indoor_outdoor), 시간단서(time_hint: 오전/오후/저녁/밤 등), "
        "장소단서(place_hint: 보이면 한 단어), 공간관계(space_relations: 배경·거리감·시선방향 등 간략히), 흐름단서(flow: 이동/머무름 등)\n"
    ...
    for data_url in sorted_images:
        url = (
            ...
            else f"data:image/jpeg;base64,{data_url}"
        )
        content.append(
            {
                "type": "image_url",
    ...
    try:
        ...
        for f in frames:
            ...
            f["elements"] = [
                clean_inline(x) for x in (f.get("elements") or []) if x
            ]
...
def draft_diary(
    ...
    if len(date_sequence) > 1:
        for i in range(1, len(date_sequence)):
            ...
            if a and b and a != b:
                ...
                if days_diff >= 1:
                    date_changes.append(
                        {"position": i + 1, "days_diff": days_diff}
                    )
    ...
    if date_changes:
        ...
        for dc in date_changes:
            if dc["days_diff"] == 1:
                date_context += (
                    f"- {dc['position']}번 사진부터: 다음 날\n"
                )
            elif dc["days_diff"] == 2:
                date_context += (
                    f"- {dc['position']}번 사진부터: 이틀 뒤\n"
                )
            elif dc["days_diff"] == 3:
                date_context += (
                    f"- {dc['position']}번 사진부터: 사흘 뒤\n"
                )
            else:
    ...
    for f in frames:
        ...
        if s:
            parts.append(s)
        if io_flag and io_flag != "unknown":
            parts.append(f"({io_flag})")
        if tm and tm != "불명":
            parts.append(f"[{tm}]")
        if ph:
            parts.append(f"#{ph}")
    ...
    length_rule = (
        "5~7문장"
        if (category_hint == "journey_multi" or len(frames) > 1)
        else "3~4문장"
    )
...
def select_draft_via_cross_validation(
    analysis: dict, tone: str, category_hint: str
) -> tuple[str, dict]:
    primary = draft_diary(
        analysis, tone, category_hint, text_model=MODEL_TEXT
    )
    used = "primary"
    alt = None
    debug = {
...
@login_required
def api_auto_diary(current_user):
    try:
        # 1) multipart/form-data 방식
        if len(request.files) > 0:
            tone = request.form.get("tone") or "중립"
            photos = json.loads(
                request.form.get("photosSummary") or "[]"
            )
            ...
            for idx, f in enumerate(files[:MAX_IMAGES]):
                ...
                orig_name = secure_filename(
                    f.filename or f"upload_{uuid.uuid4().hex}"
                )
                ...
                if final_dt:
                    ts_ms = int(final_dt.timestamp() * 1000)
                    img_dict["takenAt"] = final_dt.isoformat(sep=" ")
                    img_dict["timestamp"] = ts_ms
                    img_dict["shotAt"] = ts_ms
                    img_dict["order_ts"] = ts_ms
                    debug_injected.append(
                        {
                            "i": idx,
                            "source": "exif|ps|name",
                            "takenAt": img_dict["takenAt"],
                        }
                    )
                else:
                    debug_injected.append(
                        {"i": idx, "source": "none", "takenAt": ""}
                    )
            ...
            category_hint = (
                "journey_multi"
                if (analysis and len(analysis.get("frames") or []) > 1)
                else "general_single"
            )

            selected_draft, cv_debug = select_draft_via_cross_validation(
                analysis, tone, category_hint
            )
            final_text = refine_diary(
                analysis, selected_draft, tone, category_hint
            )
                ...
                return jsonify(
                    {
                        ...
                        "used": "vision-3stage",
                        "observations": (analysis or {}).get(
                            "frames", []
                        ),
                        "ordering_debug": (analysis or {}).get(
                            "ordering_debug", []
                        ),
                        "date_sequence": (analysis or {}).get(
                            "date_sequence", []
                        ),
        ...
        debug_meta_head = [
            {
                "i": i,
                "shotAt": (images_meta[i] or {}).get("shotAt"),
            }
            for i in range(min(len(images_meta), len(images_raw)))
        ]
                ...
                if shot is not None:
                    try:
                        ...
                        if isinstance(shot, (int, float)):
                            ...
                            ts_ms = (
                                shot_int
                                if shot_int > 10_000_000_000
                                else shot_int * 1000
                            )
                            dt_val = datetime.fromtimestamp(ts_ms / 1000.0)
                        elif isinstance(shot, str):
                            dt_val = _parse_any_dt(shot)
                            if dt_val:
                                ts_ms = int(dt_val.timestamp() * 1000)

                        if dt_val and ts_ms is not None:
                            item["takenAt"] = dt_val.isoformat(sep=" ")
                        ...
                        debug_injected.append(
                            {
                                "i": i,
                                "source": "shotAt_exception",
                                "err": str(e),
                            }
                        )
                else:
                    debug_injected.append(
                        {"i": i, "source": "no_shotAt"}
                    )
            else:
        ...
        if images:
            try:
                analysis = analyze_images(
                    images, photos_summary=photos
                )
                category_hint = (
                    "journey_multi"
                    if (
                        analysis
                        and len(analysis.get("frames") or []) > 1
                    )
                    else "general_single"
                )

                selected_draft, cv_debug = (
                    select_draft_via_cross_validation(
                        analysis, tone, category_hint
                    )
                )
                final_text = refine_diary(
                ...
                return jsonify(
                    {
                        "ok": True,
                        "body": text,
                        "category": decide_category_from_lines(
                            lines
                        ),
                        "used": "summary-lines",
                        "observations": lines,
...
if __name__ == "__main__":
    print("\n===========================================")
    print("Snaplog server 시작 → http://127.0.0.1:5000")
